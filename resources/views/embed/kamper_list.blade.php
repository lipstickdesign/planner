@php use Carbon\Carbon; $grouped = collect($kamper)->groupBy('date'); @endphp
<style>
...
@else
  @foreach($grouped as $date => $matches)
    @php $head = $date ? ucfirst(Carbon::parse($date)->locale('nb')->isoFormat('dddd D. MMMM')) : ''; @endphp
    <div class="vk-day">
      <div class="vk-dh">{{ $head }}</div>
