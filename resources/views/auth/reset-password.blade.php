...
<title>Lag nytt passord – Vivu Planner</title>
...
<div class="card">
  ...
  <p>Velg et nytt passord for kontoen din (minst 8 tegn).</p>
  @if ($errors->any())<div class="err">{{ $errors->first() }}</div>@endif
  <form method="POST" action="{{ route('password.update') }}">
    @csrf
    <input type="hidden" name="token" value="{{ $token }}">
    <input type="email" name="email" placeholder="E-post" value="{{ old('email', request('email')) }}" required autofocus>
    <input type="password" name="password" placeholder="Nytt passord" required>
    <input type="password" name="password_confirmation" placeholder="Gjenta passord" required>
